...
ae_freq = alt.Chart(subset).mark_bar().encode(
    ...
    alt.Color('sex:N')
    ).properties(title = f'{vaccine} vaccine {seriousness} adverse event frequency, filtered by vaccine, stratified by age and sex')
...
ae_prop = alt.Chart(subset).mark_bar(
        ...
        alt.Column('age:O', header = alt.Header(titleOrient = 'bottom', labelOrient = 'bottom', labelColor = 'white'), title = ''),
        alt.Color('sex:N')
    ).properties(title = f'{vaccine} vaccine {seriousness} adverse event percentage, filtered by vaccine, stratified by age and sex')
